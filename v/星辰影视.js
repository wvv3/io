...
说明：无需分类ID，强制影视漫默认热度排序，恒轩单线路+水星优先，移除推荐短剧
"ext": {
    "host": "xxxx", //站点网址
...
async function home(filter) {
    try {
        ...
        let classes = typeArr.map((it,idx) => {
            ...
            const isHotCate = cName.includes('电视剧') || cName.includes('电影') || cName.includes('动漫');
            return {type_name: cName, type_id: cId, isHotCate};
        });
        
        classes = classes.filter(item => !['推荐', '短剧'].includes(item.type_name));
        
        const tvCate = classes.find(item => item.type_name.includes('电视剧'));
        const movieCate = classes.find(item => item.type_name.includes('电影'));
        const otherCates = classes.filter(item => !item.type_name.includes('电视剧') && !item.type_name.includes('电影'));
        classes = [];
        tvCate && classes.push(tvCate);
        movieCate && classes.push(movieCate);
        otherCates.length && classes.push(...otherCates);
        
        const returnClasses = classes.map(item => ({type_name: item.type_name, type_id: item.type_id}));
        
        KParams.cateMarks = classes.map(item => ({type_id: item.type_id, isHotCate: item.isHotCate}));
        
        if (KParams.catesSet) {returnClasses = ctSet(returnClasses, KParams.catesSet);}
        let filters = {};
        try {
        ...
        return JSON.stringify({class: returnClasses, filters: filters});
    } catch (e) {
...
async function category(tid, pg, filter, extend) {
    try {
        ...
        const cateMark = KParams.cateMarks?.find(item => item.type_id === tid) || {isHotCate: false};
        if (cateMark.isHotCate) {
            
            let typeItem = await request(`${HOST}/vs/${tid}-----------.html`);
            let hotSortVal = cutStr(typeItem, 'hl-rb-title', '</div>', '', false).match(/\d+--([^-]+)-/)?.[1] ?? '';
            fl.by = hotSortVal; 
        }
...
async function detail(ids) {
    try {
        ...
        let useUrl = '';
        const mercuryIdx = ktabs.findIndex(tab => tab.includes('水星'));
        if (mercuryIdx > -1 && kurls[mercuryIdx] && kurls[mercuryIdx].trim() !== '') {
            useUrl = kurls[mercuryIdx];
        } else {
            useUrl = kurls.find(url => url && url.trim() !== '') || '';
        }
        ktabs = ['恒轩'];
        kurls = [useUrl];
...
async function request(reqUrl, options = {}) {
    try {
        ...
        if (options.withHeaders) {
            const resHeaders = typeof res.headers === 'object' && !Array.isArray(headers) && res.headers ? res.headers : {};
