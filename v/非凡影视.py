...
class Spider(T):
	...
	def detailContent(I,did):
		...
		try:V=C.get(f"{I.home_url}/api.php/provide/vod?ac=detail&ids={U}",headers=I.headers);B=V.json()[A][0];K.append({J:B[J],D:B[D],E:B[E],F:B[F],L:B[L],M:B[M],N:B[N],P:B[P],Q:B[Q],R:'恒轩',S:B[S].split(T)[-1]})
