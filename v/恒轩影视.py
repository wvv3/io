    ...
    def detailContent(self, array):
        try:
            ...
            for tab in root.xpath("//ul[contains(@class,'nav-tabs')]/li"):
                ...
                if tab_name and tab_id and play_tab_count < 2: 
                    ...
                    if play_list:
                        ...
                        for episode in play_list:
                            ...
                            if ep_url:
                                if play_tab_count == 1:
                                    tab1_episodes.append({"name": ep_name, "url": ep_url})
                                else:
                                    tab2_episodes.append({"name": ep_name, "url": ep_url})
            
          
            if tab2_episodes:
               
                for ep in tab2_episodes:
                    
                    all_episodes.append(f"{ep['name']}${ep['url']}|")
            else:
               
                for ep in tab1_episodes:
                    all_episodes.append(f"{ep['name']}${ep['url']}|")
            ...
            if all_episodes:
                ...
                for ep in all_episodes:
                    ...
                    if len(parts) == 2:
                        ...
                        if urls.endswith("|") and len(urls) > 1:
                            urls = urls[:-1]
                        processed_episodes.append(f"{ep_name}${urls}")
